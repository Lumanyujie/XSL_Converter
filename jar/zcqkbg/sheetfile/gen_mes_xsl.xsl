...
<xsl:variable name="mes-root"
  select="concat($gdata,'/messages/mes_xsl/languages/',$language)"/>

<xsl:variable name="header-root"
  select="concat($gdata,'/messages/mes_xsl/headers')"/>

<xsl:variable name="shelf-life-file"
  select="concat($gdata,'/messages/msg_shelf_life.xml')"/>
...
<!--
        </xsl:for-each> --> <!-- $modifiers/number-stream -->
<!--
      </xsl:element> --> <!-- xsl:template -->
<!--
    </xsl:element> --> <!-- stylesheet -->
<!--
  </xsl:result-document> --> <!-- modifiers stylesheet -->

<!--
</xsl:template> --> <!-- write-default-formats -->
...
</xsl:template> <!-- make-header-template -->
...
</xsl:template> <!-- make-subject-template -->
...
</xsl:template> <!-- make-message-template -->
...
</xsl:template> <!-- process-section old -->
<!--
...
<xsl:template name="process-line-old">
  ...
  <xsl:for-each select="$context/node()">
    ...
    <xsl:choose>
      ...
      </xsl:when> <!-- formatting data is present -->
  ...
  </xsl:for-each> <!--$context/node()  -->

</xsl:template> <!-- process-line-old -->
...
<xsl:template name="process-data-item">
  ...
  <xsl:variable name="this_item">
    ...
    </xsl:choose> <!-- the type of variable (literal, function, or argument ref -->
  </xsl:variable>
...
</xsl:template> <!-- process-data-item -->
...
<xsl:template name="process-function">
  ...
  <xsl:element name="xsl:call-template">
    ...
    <xsl:choose>
      <xsl:when test="$is-voc-fun = 'true'"> <!-- It is a vocab function. -->
      ...
      <xsl:otherwise> <!-- Its a normal function, get the function name. -->
        <xsl:attribute name="name">
    ...
    </xsl:for-each> <!-- end for each argument -->
...
<xsl:template name="get-context-prefix">
  ...
  <xsl:choose>
   ...
   <!-- Replace each remaining level of the context with "../" (for
    ...
    </xsl:otherwise> <!-- the remaining path does not contain a period -->

  </xsl:choose>

</xsl:template> <!-- get-context-prefix -->
...
</xsl:template> <!-- create-number-stream-template -->
...
<xsl:template name="ws-function">
    ...
    </xsl:element><!-- end template stop-table -->
    ...
    </xsl:element><!-- end template process-br -->

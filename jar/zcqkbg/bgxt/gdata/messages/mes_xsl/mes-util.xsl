...
-->
	...
	<xsl:variable name="LOCATION_FORMAT_DEF">
		<!--<xsl:text>&lt;a href="#" class='secl'&gt;</xsl:text>-->
...
-->
	...
	<xsl:template name="render-header">
		...
		<xsl:text>&lt;span class='secl'&gt;</xsl:text>
		...
		<xsl:text>演习课题：XXXXXXX演习</xsl:text>
		
		
		<xsl:text>&lt;/div&gt;</xsl:text>
		<xsl:text>&lt;div class='playcode'&gt;</xsl:text>
		<xsl:text>报告序号：红方XXXXXXXX</xsl:text>

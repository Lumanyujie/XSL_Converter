...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
   ...
   <xsl:template name="render-body">
      ...
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">飞机战损情况</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="display_cur_text">
         <xsl:with-param name="in_string">
      ...
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">定期报告&lt;/p&gt;</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      ...
      <xsl:for-each select="S1">
         ...
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">飞机战损明细&lt;/p&gt;</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="save-header">
            <xsl:with-param name="header-string">机型|出动架次|空战损失|防空损失|油料损失|着陆损失| 空地损失|炮火损失|战斗损失|其他损失</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
      ...
      <xsl:for-each select="S3">
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">&lt;p class='table_name'&gt;飞机战损统计&lt;/p&gt;</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="save-header">
            <xsl:with-param name="header-string">机型|出动架次|空战损失|防空损失|油料损失|着陆损失| 空地损失|炮火损失|战斗损失|其他损失</xsl:with-param>
      ...
      <xsl:for-each select="S4">
         <xsl:call-template name="process-br"/>
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">本方无飞机损失。</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
      </xsl:for-each>
   </xsl:template>

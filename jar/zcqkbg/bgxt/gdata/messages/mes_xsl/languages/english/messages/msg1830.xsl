...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
   ...
   <xsl:template name="render-body">
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">&lt;p class='title'&gt;使用此舰船部队原型 </xsl:with-param>
      ...
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">)的部队不同态势下消耗补给物资总数&lt;/p&gt;</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="process-br"/>
      <xsl:call-template name="save-header">
         <xsl:with-param name="header-string"> 物资名称 | 物资编号 | 正常作战 | 机动 | 空中作战 | 提供舰炮火力 | 计算单位</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="process-br"/>
      <xsl:for-each select="G1/R">
         ...
         <xsl:variable name="out-string">
            <xsl:call-template name="format-string-right">
               <xsl:with-param name="str">
                  <xsl:call-template name="get-vocab-value">
                     <xsl:with-param name="vocab">Consumption_Type</xsl:with-param>
                     <xsl:with-param name="arg1">
                        <xsl:value-of select="D/@A"/>
                     </xsl:with-param>
                  </xsl:call-template>
      ...
      <xsl:call-template name="process-br"/>
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">  该列表中不包含已使用的补给、</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="process-br"/>
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">  为没有基本携行量的舰船原型的补给和已运到战区的补给。</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="process-br"/>
      <xsl:for-each select="S1">
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">  舰船原型不允许基于态势的消耗量的补给。</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
      </xsl:for-each>

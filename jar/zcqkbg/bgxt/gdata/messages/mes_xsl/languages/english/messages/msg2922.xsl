...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
   ...
   <xsl:template name="render-body">
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">&lt;p cless='title'&gt;来自</xsl:with-param>
      ...
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">的敌方情报&lt;/p&gt;</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="save-header">
         <xsl:with-param name="header-string"> 敌方作战单位 |上级指挥级别 |所属国家| 部队类型|军种| 部队位置</xsl:with-param>
      </xsl:call-template>

...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
   ...
   <xsl:template name="render-body">
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">&lt;p class='title'&gt;空中机群（代字：</xsl:with-param>
      ...
      <xsl:for-each select="S2">
         ...
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">在该空中机群中，以下编队不能在计划临空时间</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="parse-col">
         ...
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">）时抵达：</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">&lt;p class='table_name'&gt;不能按时到达编队名单&lt;/p&gt;</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="save-header">
            <xsl:with-param name="header-string">编队批号|最早临空时间</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
         ...
         <xsl:call-template name="process-br"/>
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">如果机群临空时间窗口不变，以上编队将自动取消。</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
      </xsl:for-each>
   </xsl:template>

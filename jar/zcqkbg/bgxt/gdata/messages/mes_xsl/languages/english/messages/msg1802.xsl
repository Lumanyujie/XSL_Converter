...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
   ...
   <xsl:template name="render-body">
      ...
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">)作战系统战术部队原型参数&lt;/p&gt; </xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="process-br"/>
      <xsl:call-template name="process-br"/>
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">  使用战术部队原型</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="display_cur_text">
         <xsl:with-param name="in_string">
      ...
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">)的部队有下列系统特性:</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="process-br"/>
      <xsl:call-template name="process-br"/>
      <xsl:for-each select="S1">
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">&lt;p class='paragraph'&gt;作战系统原型</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="display_cur_text">
            <xsl:with-param name="in_string">
      ...
      <xsl:for-each select="S2">
         <xsl:call-template name="save-header">
            <xsl:with-param name="header-string"> 作战系统|编号| 编制装备表系统</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>

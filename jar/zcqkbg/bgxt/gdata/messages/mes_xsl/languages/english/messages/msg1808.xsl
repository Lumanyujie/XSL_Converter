...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
   ...
   <xsl:template name="render-body">
      ...
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">)在不同态势下消耗的补给物资数量&lt;/p&gt;</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="process-br"/>
      <xsl:call-template name="process-br"/>
      <xsl:call-template name="save-header">
         <xsl:with-param name="header-string">名称 | 进攻| 防御| 阻击 | 撤退| 消耗单位 |消耗</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="process-br"/>
      ...
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">  注意：该列表不包含任何已使用补给或任何战术部队原型没有基本载运量或任何运抵战区的补给。</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="process-br"/>
      <xsl:for-each select="S1">
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">  该战术部队原型没有许可基于态势消耗值的补给。</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
      </xsl:for-each>

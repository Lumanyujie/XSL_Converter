...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
   ...
   <xsl:template name="render-body">
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">&lt;p class='title'&gt;  面杀伤武器杀伤能力</xsl:with-param>
      ...
      <xsl:for-each select="S1">
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">  对于给定的面杀伤毁伤指标，期望目标类型至少有一个致命区域按要求被改变。</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
      </xsl:for-each>
      <xsl:for-each select="S2">
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">  对作战武器目标，使用毁伤指标</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="display_cur_text">
            <xsl:with-param name="in_string">
         ...
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">实施一次单轮突击，其杀伤面积按子类划分如下：</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
         <xsl:for-each select="G1/R">
            <xsl:call-template name="save-header">
               <xsl:with-param name="header-string">目标子类| 杀伤面积:</xsl:with-param>
            </xsl:call-template>
            <xsl:call-template name="process-br"/>
            <xsl:call-template name="parse-col">
            ...
            <xsl:variable name="out-string">
               <xsl:call-template name="format-string-left">
                  ...
                  <xsl:with-param name="trim" select="26"/>
               </xsl:call-template>
            </xsl:variable>
            <xsl:call-template name="display_cur_text">
            ...
            <xsl:call-template name="parse-col">
               <xsl:with-param name="target-string">平方米 </xsl:with-param>
               <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
            </xsl:call-template>
            <xsl:call-template name="process-br"/>
         </xsl:for-each>
      ...
      <xsl:for-each select="S3">
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">  对补给目标，使用毁伤指标</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="display_cur_text">
            <xsl:with-param name="in_string">

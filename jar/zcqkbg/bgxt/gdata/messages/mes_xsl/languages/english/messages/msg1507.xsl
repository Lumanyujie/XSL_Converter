...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
   ...
   <xsl:template name="render-body">
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">&lt;p class='title'&gt; 通用海军参数&lt;/p&gt;</xsl:with-param>
      ...
      <xsl:for-each select="S1">
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">  只有补给物资达到申请量的</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         ...
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">时，海上补给才会开始。</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
      </xsl:for-each>
      <xsl:for-each select="S2">
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">  只有部队在</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="display_cur_text">
            <xsl:with-param name="in_string">
         ...
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">天（最大两栖行经时间）内能够行进舰船与突击/登录点间路程， </xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">舰船或部队的两栖登陆船才会在突击/登陆行动中被使用。</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
      </xsl:for-each>
      <xsl:for-each select="S3">
         ...
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">是参加突击/抢滩行动的舰船距离出发地的最大距离(最大海滩行动距离)。</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
      </xsl:for-each>
      <xsl:for-each select="S4">
         ...
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">是参加两栖行动要求使用海滩或车辆资源的舰船自动驶入的最大距离（最大的舰船移动距离)。</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
      </xsl:for-each>
      <xsl:for-each select="S5">
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">  电子侦察监视（ESM）报告标志被设置为：</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="display_cur_text">
            <xsl:with-param name="in_string">
      ...
      <xsl:for-each select="S6">
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">  关于新深水区。新区域最大深度未被接受的原因：</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">区域范围无效(区域未重叠)或者区域最大深度</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">超过了允许的最大水深。然而为该区域指定的新的机动时间倍增系数被接受。</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
      </xsl:for-each>
      <xsl:for-each select="S7">
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">&lt;p class='table_name'&gt; 当前深水区属性设置表&lt;/p&gt;</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="save-header">
            <xsl:with-param name="header-string">区域|区域名称|最大深度|机动时间倍增系数</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>

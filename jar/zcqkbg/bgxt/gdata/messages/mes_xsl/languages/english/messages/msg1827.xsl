...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
   ...
   <xsl:template name="render-body">
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">&lt;p class='title'&gt;  使用舰船部队原型</xsl:with-param>
      ...
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">)部队的补给类总量 &lt;/p&gt;</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      ...
      <xsl:for-each select="S1">
         <xsl:for-each select="G1/R">
            ...
            <xsl:call-template name="parse-col">
               <xsl:with-param name="target-string">)送达战区:  </xsl:with-param>
               <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
            </xsl:call-template>
            <xsl:variable name="out-string">
               <xsl:call-template name="format-string-right">
      ...
      <xsl:for-each select="S2">
         <xsl:for-each select="G1/R">
            ...
            <xsl:call-template name="parse-col">
               <xsl:with-param name="target-string">)重新调整的补给等级:  </xsl:with-param>
               <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
            </xsl:call-template>
            <xsl:variable name="out-string">
               <xsl:call-template name="format-string-right">
      ...
      <xsl:for-each select="S4">
         <xsl:for-each select="G1/R">
            ...
            <xsl:call-template name="parse-col">
               <xsl:with-param name="target-string">)装载量等级:  </xsl:with-param>
               <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
            </xsl:call-template>
            <xsl:variable name="out-string">
               <xsl:call-template name="format-string-right">

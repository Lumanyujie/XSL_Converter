...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
   ...
   <xsl:template name="render-body">
      ...
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">运输工具损失情况</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="display_cur_text">
         <xsl:with-param name="in_string">
      ...
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">&lt;p class='table_name'&gt;本时段运输工具损失明细&lt;/p&gt;</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="save-header">
         <xsl:with-param name="header-string">运输工具类型|遭敌地面攻击|遭敌空袭|遭敌炮击|合计</xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="process-br"/>

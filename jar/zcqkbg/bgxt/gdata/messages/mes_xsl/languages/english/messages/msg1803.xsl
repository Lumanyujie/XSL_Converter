...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
   ...
   <xsl:template name="render-body">
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">&lt;p class='title'&gt;  使用战术部队原型</xsl:with-param>
      ...
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">)的部队补给类总量 &lt;/p&gt;</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      ...
      <xsl:for-each select="S1">
         <xsl:call-template name="save-header">
            <xsl:with-param name="header-string"> 种类 | (编号) | 补给至战区</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
      ...
      <xsl:for-each select="S2">
         <xsl:call-template name="save-header">
            <xsl:with-param name="header-string"> 种类 | (编号) | 重新安排的补给标准</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
         <xsl:for-each select="G1/R">
            <xsl:call-template name="parse-col">
      ...
      <xsl:for-each select="S3">
         <xsl:call-template name="save-header">
            <xsl:with-param name="header-string">种类 (编号) | 库存目标</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
         <xsl:for-each select="G1/R">
            <xsl:call-template name="parse-col">
      ...
      <xsl:for-each select="S4">
         <xsl:call-template name="save-header">
            <xsl:with-param name="header-string"> 种类 (编号) | 基本运载量等级</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="process-br"/>
         <xsl:for-each select="G1/R">
            <xsl:call-template name="parse-col">

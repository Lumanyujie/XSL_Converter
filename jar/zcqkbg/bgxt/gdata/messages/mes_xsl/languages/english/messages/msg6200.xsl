...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
   ...
   <xsl:template name="render-body">
      ...
      <xsl:call-template name="parse-col">
         <xsl:with-param name="target-string">作战情况报告&lt;/p&gt;</xsl:with-param>
         <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
      </xsl:call-template>
      <xsl:for-each select="S1">
         ...
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">对以下敌海上部队发起自动攻击：</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
      ...
      <xsl:for-each select="S2">
         ...
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">发起自动攻击。</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
      </xsl:for-each>
      <xsl:for-each select="S3">
         ...
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">的自动交火终止， </xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">因为其地面交战规则已撤销。</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
      </xsl:for-each>
      <xsl:for-each select="S5">
         ...
         <xsl:call-template name="parse-col">
            <xsl:with-param name="target-string">正遭到某海军舰载武器的自动攻击。</xsl:with-param>
            <xsl:with-param name="is-begin-with-slide">0</xsl:with-param>
         </xsl:call-template>
      </xsl:for-each>
   </xsl:template>
